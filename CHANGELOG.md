...
### Changed
- Version bump only — no functional changes. Required because version `0.5.2` was already submitted to AMO's unlisted channel and cannot be reused for the listed channel.
...
### Added
- `browser_specific_settings.gecko.data_collection_permissions` declaring `required: ["none"]`. Mozilla now requires this field for all new AMO submissions; the extension does not collect any user data, so `"none"` is accurate.
